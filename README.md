...
<img src="https://ws1.sinaimg.cn/large/006tNbRwly1fvae8xydi9j310u140ad6.jpg" width="400px" height="400px" align="center" />
...
根据sorted set排好序的键值，建立索引文件和数据文件，索引文件每个桶只有一份即index_Thread1_01，记录一千条数据的key_start和key_end（之后查找会根据要查找的键是否在这个范围内判断是否需要读取这一块）

建立好索引以后，索引的写入和数据的写入都是交给线程池去做的，利用reentrantlock和callable返回成功状态。同时将写好的索引和数据都放到hdfs中。

#### 方案二

不使用排序，二级索引使用B+树。但是具体和磁盘如何交互这边不太好确定。
...
1. 如果每次写入的时候是写入1000条，每条因为Key不超过20B，value不超过10KB，所以大致写入的时候是10MB，读的时候也是10MB读取。一个block的大小是64MB，这样会存在很多小文件，对namenode带去压力，如何解决？
   ...
   **第一、**使用hadoop archive归档命令进行归档，相当于在原来的namenode的元数据之上加了一层索引。hadoop通过二级索引进行文件的查找，所以效率上会比原来的慢一些。

   **第二、**在项目当中已经做了适当的优化，因为是每1000次才进行写入操作。

   **第三、**使用第三方的数据访问模式进行数据的管理，比如HBase。通过HBase而不是Hadoop写入，HBase的写入方式类似于sequenceFile，是连续的key-value对。
